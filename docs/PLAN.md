...
Прототип реализован и развёрнут:
...
| Клиентский стор | Zustand v5 — только то, что нужно вне React | звук и метрики читаются пулом в колбэках через `getState()` |
...
Основа: Next.js 16 (App Router, React 19 + Compiler), TypeScript,
Feature-Sliced Design. Тесты: Vitest + Playwright.
...
жестового кода в приложении нет. Так же устроены реальные веб-реализации
TikTok-лент.

К платформе пришли через практику, а не сразу. Кандидаты:
...
- **Swiper Virtual** — каталог открытых багов ровно в нужной связке
  (vertical + Virtual + Mousewheel), React-обёртка объявлена к удалению.
- **Flicking (Naver)** — единственный движок со строгим «1 за жест» и
  ограниченным DOM; был реализован полностью. Тач работал отлично, но ядро не
  обрабатывает колесо, а каждый мост к нему (плагин, отсутствующий в
  актуальной версии; подключение к внутреннему Axes; синтетический drag)
  требовал самописных эвристик по инерции macOS-тачпада — стабильности они не
  дали. Отсюда архитектурный принцип: жесты колеса и тачпада обрабатывает
  браузер.
...
- Полосу активному гарантирует малый буфер соседей. Явную паузу соседних
  загрузок при ребуфере активного реализовали и сняли: `stopLoad()` до разбора
  манифеста замораживает hls.js необратимо — случай, когда оптимизация опаснее
  проблемы, которую решает.
...
| Zustand | метрики воспроизведения | пишет пул из медиа-событий, читает debug-оверлей |
| Локальный стейт виджета | `activeIndex`, `settled` | один потребитель; умирает с компонентом при навигации |
...
5. **Deep-links** (`/[id]` + SSR + OG-теги) — прямой следующий шаг
   архитектуры; у TikTok web ссылки на элемент ленты работают через раз.
...
flowchart TD
    ...
    PG --> W["widgets/video-feed — скроллер, панель, действия, метрики; use-feed-drag; preload-manager"]
...
- [`?debug=1`](https://tiktok.dealunloker.com/?debug=1) — TTFF / ребуферы /
  dropped frames.
- `pnpm test` — юнит-тесты: пул (гонки, LRU, ошибки, mute-политика),
  preload-manager (дебаунс, бюджеты, abort), сторы, генератор, компоненты.
- `pnpm test:e2e e2e/feed.spec.ts` — e2e ленты в реальном Chromium.
